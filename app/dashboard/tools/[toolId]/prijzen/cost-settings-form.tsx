...
import type { AccountDefaults } from "@/app/lib/tools";
...
export function CostSettingsForm({
  ...
  const [state, formAction, pending] = useActionState<
    ...
    FormData
  >(updateCostSettingsAction.bind(null, toolId), null);
  ...
  return (
    <form action={formAction} className="flex flex-col gap-5">
      ...
      <CostTypeCard
        title="Arbeidskosten"
        description="De standaardtarieven voor arbeidstijd van producten in deze tool."
        enabled={arbeidOn}
      ...
      >
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <AccountDefaultField
            ...
            htmlFor="arbeidTariefUur"
            valueName="arbeidTariefUur"
            useAccountName="gebruiktAccountArbeidTarief"
            useAccountDefault={costSettings.gebruiktAccountArbeidTarief}
            accountValue={accountDefaults.standaardArbeidTariefUur}
            defaultValue={costSettings.arbeidTariefUur}
            suffix="/ uur"
            error={fieldErrors?.arbeidTariefUur}
          />
          <Field
      ...
      >
        <FieldRow>
          <AccountDefaultField
            label="Voorrijkosten"
            htmlFor="voorrijTarief"
            valueName="voorrijTarief"
            useAccountName="gebruiktAccountVoorrijTarief"
            useAccountDefault={costSettings.gebruiktAccountVoorrijTarief}
            accountValue={accountDefaults.standaardVoorrijTarief}
            defaultValue={costSettings.voorrijTarief}
            error={fieldErrors?.voorrijTarief}
          />
        </FieldRow>
        <p className="text-xs text-muted-foreground">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Algemene instellingen voor deze rekentool.</CardDescription>
        </CardHeader>
        <CardContent>
          <FieldRow>
            <AccountDefaultField
              ...
              valueName="btwPercentage"
              useAccountName="gebruiktAccountBtw"
              useAccountDefault={costSettings.gebruiktAccountBtw}
              accountValue={accountDefaults.standaardBtwPercentage}
              accountSuffix="%"
...
// "Gebruik accountinstelling" / "Eigen waarde" — het defaults-cascade-patroon
// uit Levering A (zie resolveCostSettings in app/lib/tools.ts). Het invoerveld
// blijft bewust altijd interactief (nooit disabled): een uitgeschakeld veld
// stuurt in een gewoon HTML-formulier geen waarde mee, en de kolom moet altijd
// een concrete "laatst bekende" waarde behouden voor het geval de vakman later
// weer naar "eigen waarde" overschakelt. Bij "gebruikt account" telt de
// ingevulde waarde alleen niet mee — dat maakt de gedimde stijl hieronder
// duidelijk.
function AccountDefaultField({
  label,
  htmlFor,
  valueName,
  useAccountName,
  ...
  const [gebruiktAccount, setGebruiktAccount] = useState(useAccountDefault);
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <span className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1">
        ...
        <label className="flex items-center gap-1.5 text-xs text-muted-foreground">
          <input
            type="checkbox"
            name={useAccountName}
            checked={gebruiktAccount}
            onChange={(e) => setGebruiktAccount(e.target.checked)}
            className="h-3.5 w-3.5 rounded border-input accent-primary"
          />
          Gebruik accountinstelling ({symbol ? formatCurrency(accountValue) : `${accountValue}${accountSuffix}`})
        </label>
      </span>
      <div className={cn(gebruiktAccount && "opacity-50")}>
        <CurrencyInput
          id={htmlFor}
          name={valueName}
      ...
      {gebruiktAccount ? (
        <p className="text-xs text-muted-foreground">Volgt de accountinstelling — vink uit voor een eigen waarde.</p>
      ) : (
